...
- Tooltip readability rule: all explanatory hover text must use the centralized `TOOLTIP_BG` / `TOOLTIP_FG` pair and `TOOLTIP_QSS` from `src/gui/theme.py`; never set tooltip foreground or background in a feature module. On Windows, Qt/qfluentwidgets can split ownership of tooltip foreground and background, so `build_qss()` must synchronize both QSS and `QApplication`'s `ToolTipBase` / `ToolTipText` palette roles. A container with its own `setStyleSheet()` can outrank the application stylesheet for descendant tooltips, so top-level feature containers (such as content/sidebar) must append `TOOLTIP_QSS` to their local stylesheet. Keep this pair high-contrast and test both values in the generated QSS.
